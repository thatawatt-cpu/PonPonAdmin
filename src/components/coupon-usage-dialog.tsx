...
async function getCouponUsages(couponId: string) {
  ...
  return usageArray(data).map(normalizeCouponUsage);
}
  ...
  useEffect(() => {
    ...
    void Promise.resolve()
      .then(() => {
        ...
        return getCouponUsages(couponId);
      })
      .then((data) => {
      ...
      <DialogContent className="max-w-4xl">
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-border bg-muted/20 px-6 py-10 text-center">
            <p className="text-sm font-semibold">ยังไม่มีการใช้คูปองนี้</p>
            <p className="mt-1 text-sm text-muted-foreground">
              เมื่อมีออเดอร์ใช้คูปอง ระบบจะแสดงเลขออเดอร์ ลูกค้า วันที่ใช้ และยอดส่วนลดที่นี่
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="sticky top-0 z-10 grid gap-3 bg-background pb-2 sm:grid-cols-3">
              ...
              <UsageSummary label="คูปอง" value={couponCode} />
            </div>
...
function normalizeCouponUsage(value: unknown): CouponUsage {
  ...
  return {
    ...
    couponCode: stringValue(record, "couponCode", "CouponCode", "code", "Code"),
    couponName: stringValue(record, "couponName", "CouponName", "name", "Name"),
